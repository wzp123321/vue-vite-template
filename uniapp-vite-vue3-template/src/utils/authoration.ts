import { SESSION_KEY_TIME_DIFF,SESSION_KEY_LOGINNAME } from '@/config/session-key';
...
export const FGetAuthorization = () => {
  ...
  const authorization = `${new Date().getTime() + dValue}_${getRandomSixNum()}_${FGetStorageData(SESSION_KEY_LOGINNAME)}`;
